-- Safe database migration for MySQL
-- Creates tables if they don't exist and migrates data safely
-- Run check_existing_schema.sql first to see current state

-- Step 1: Create game_attributes table (this will be our main consolidated table)
CREATE TABLE IF NOT EXISTS game_attributes (
    id INT PRIMARY KEY AUTO_INCREMENT,
    category VARCHAR(100) NOT NULL,
    name VARCHAR(255) NOT NULL,
    value VARCHAR(255) NOT NULL,
    type VARCHAR(50) NOT NULL DEFAULT 'attribute',
    ...
    INDEX idx_game_attributes_type (type),
    INDEX idx_game_attributes_category (category),
    INDEX idx_game_attributes_type_category (type, category),
    INDEX idx_game_attributes_active (is_active),
    INDEX idx_game_attributes_weight (weight)
);

-- Step 2: Migrate data from game_assets if it exists
INSERT IGNORE INTO game_attributes (category, name, value, type, description, weight, is_active)
SELECT 
    type as category,
    name,
    name as value,
    CASE 
        WHEN type = 'climate' THEN 'environment'
        WHEN type = 'gender' THEN 'character'
        WHEN type = 'artistic_style' THEN 'visual'
        WHEN type = 'environment' THEN 'setting'
        WHEN type = 'cultural_artifact' THEN 'item'
        WHEN type = 'experience_level' THEN 'progression'
        WHEN type = 'race' THEN 'species'
        ELSE 'attribute'
    END as type,
    description,
...
FROM game_assets
WHERE EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'game_assets' AND table_schema = DATABASE());

-- Step 3: Migrate data from attributes table if it exists (and is different from game_attributes)
INSERT IGNORE INTO game_attributes (category, name, value, weight, is_active)
SELECT 
    category,
    name,
    COALESCE(value, name) as value,
    COALESCE(weight, 1) as weight,
    COALESCE(is_active, TRUE) as is_active
FROM attributes
WHERE EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'attributes' AND table_schema = DATABASE())
AND NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'game_attributes' AND table_schema = DATABASE() AND table_name = 'attributes');

-- Step 4: Create unified species table
...
-- Step 5: Migrate existing species data if table exists
...
SELECT 
    ...
    NULL as description,
    ears, tail, wings, 
    CASE WHEN features IS NOT NULL AND features != '' THEN JSON_QUOTE(features) ELSE NULL END as features,
    CASE WHEN personality IS NOT NULL AND personality != '' THEN JSON_QUOTE(personality) ELSE NULL END as personality,
...
-- Step 6: Migrate alien species data if table exists  
INSERT IGNORE INTO unified_species (
...
-- Step 7: Add comprehensive default game attribute data (only if not already present)
...
-- Step 9: Create attributes view for compatibility (points to game_attributes)
CREATE OR REPLACE VIEW attributes AS
SELECT * FROM game_attributes;

-- Step 10: Cleanup - Only drop old tables after confirming migration worked
...
-- DROP VIEW IF EXISTS species; 
-- DROP VIEW IF EXISTS alien_species;

-- Migration completed successfully
SELECT 'Migration completed. Check data and uncomment cleanup section when ready.' as status;